...
class MusicListActivity : ComponentActivity() {
    ...
    private fun getMusicList(): ArrayList<Song> {
        ...
        val cursor = db.query(
            ...
            arrayOf(
                MusicDatabase.COLUMN_NAME,
                MusicDatabase.COLUMN_ID,
                MusicDatabase.COLUMN_ARTIST,
                MusicDatabase.COLUMN_DURATION,
                MusicDatabase.COLUMN_DATA,
                MusicDatabase.COLUMN_MIMETYPE,
                MusicDatabase.COLUMN_SIZE,
        ...
        while (cursor.moveToNext()) {
            ...
            val duration =
                cursor.getInt(cursor.getColumnIndexOrThrow(MusicDatabase.COLUMN_DURATION))
            val data = cursor.getString(cursor.getColumnIndexOrThrow(MusicDatabase.COLUMN_DATA))
            val mimeType =
                cursor.getString(cursor.getColumnIndexOrThrow(MusicDatabase.COLUMN_MIMETYPE))
            val size = cursor.getLong(cursor.getColumnIndexOrThrow(MusicDatabase.COLUMN_SIZE))
            val albumId =
                cursor.getLong(cursor.getColumnIndexOrThrow(MusicDatabase.COLUMN_ALBUM_ID))
